...
__device__ static const int stencil_x[stencil_size] = {-1, -1, -1, 0, 0,
                                                       0,  1,  1,  1};

__device__ static const int stencil_y[stencil_size] = {-1, 0,  1, -1, 0,
                                                       1,  -1, 0, 1};

template <typename T>
void setVal(context &ctx, logical_data<slice<T>> a, T val) {
  ctx.parallel_for(a.shape(), a.write()).set_symbol("setVal")->*
      [val] __device__(size_t i, auto da) { da(i) = val; };
}

template <typename BinaryOp> struct OpWrapper {
  OpWrapper(BinaryOp _op) : op(mv(_op)){};
...
void exclusive_scan(Ctx &ctx, logical_data<slice<InT>> in_data,
                    logical_data<slice<OutT>> out_data, BinaryOp &&op,
                    OutT init_val) {
  ...
  cub::DeviceScan::ExclusiveScan(
      d_temp_storage, temp_storage_bytes, (InT *)nullptr, (OutT *)nullptr,
      OpWrapper<BinaryOp>(op), init_val, in_data.shape().size(), 0);

  auto ltemp = ctx.logical_data(shape_of<slice<char>>(temp_storage_bytes));

  ctx.task(in_data.read(), out_data.write(), ltemp.write())
          ->*[&op, init_val, nitems](cudaStream_t stream, auto d_in, auto d_out,
                                     auto d_temp) {
                size_t tmp_size = d_temp.size();
                cub::DeviceScan::ExclusiveScan(
                    (void *)d_temp.data_handle(), tmp_size,
                    (InT *)d_in.data_handle(), (OutT *)d_out.data_handle(),
                    OpWrapper<BinaryOp>(op), init_val, nitems, stream);
              };
}

void print2file(context &ctx, const logical_data<slice<double>> la,
                string path) {
...
__device__ bool stencilBoundaryCheck(int rows, int cols, int rb, int cb) {
...
__inline__ __host__ __device__ void indexFixSoft(int &ii, int &jj, int rb,
                                                 int cb) {
...
__inline__ __device__ bool coordBoundaryCheck(double xx, double yy, int rb,
                                              int cb, double Dx, double Dy) {
...
__inline__ __device__ double kernel(double dr, double ad, double h) {
  ...
  if (0 <= o && o <= 0.5) {
    return (ad / h) *
           (pow(5 / 2 - o, 4) - 5 * pow(3 / 2 - o, 4) + 10 * pow(0.5 - o, 4));
  } else if (0.5 < o && o <= 1.5) {
...
__inline__ __device__ double gradKernel(double dr, double ad, double h) {
...
__device__ bool circleCheck(double a, double b, double radius) {
...
__inline__ __device__ double viscTensor(double dvel, double difr, double mui,
                                        double muj, double rhoi, double rhoj,
                                        double deltaw, double h) {
  // Calculates viscocity tensor

  double eh = 0.01 * h * h;

  return -16 * (mui + muj) / (pow(difr, 2) * (rhoi + rhoj) + eh) *
         (dvel * difr) * deltaw;
}

void setPressure(context &ctx, logical_data<slice<double>> a,
                 logical_data<slice<double>> rho) {
  // Calculates pressure
  double gamma = 7.0;

  ctx.parallel_for(a.shape(), a.write(), rho.read()).set_symbol("setPressure")
          ->*
      [=] __device__(size_t jj, auto da, auto drho) {
        da(jj) = Rho * c0 * c0 / gamma * (pow(drho(jj) / Rho, gamma) - 1);
      };
}

__inline__ __device__ void setPressure2(slice<double> &a, double rrho,
                                        int cols) {
  // Calculates pressure
  double gamma = 7.0;
...
int main() {
  ...
  n2 = n - n1;
  double factor = L * H / V;
  int nx = sqrt(L / H * n * factor + pow((L - H), 2) / (4 * H * L)) -
           (L - H) / (2 * H);
  int ny = n * factor / nx;
  dx = L / (nx - 1);
  ...
  auto initialise = [=](context &ctx, logical_data<slice<double>> la,
                        logical_data<slice<double>> lb, double l1, double l2) {
    // Initialise flow particles
    bool randomIni = 1;
    space = row_fact * Dx;
    ctx.host_launch(la.rw(), lb.rw()).set_symbol("initialise")
            ->*[=](auto a, auto b) {
                  if (randomIni == 1) {
  ...
  auto initialiseBoundaries = [=](context &ctx, logical_data<slice<double>> lx,
                                  logical_data<slice<double>> ly, int nbh,
                                  int nbv, int nbh_side, int nbv_side) {
    // Initialise boundary particles
    ctx.host_launch(lx.rw(), ly.rw()).set_symbol("initialiseBoundaries")
            ->*[=](auto x, auto y) {
  ...
  while (t_sim < t_total) {
    ...
    if (iter_cnt % 1 == 0) {
      ...
      ctx.parallel_for(lx.shape(), lx.read(), ly.read(), lndeg.rw())
              .set_symbol("neighbors")
              ->*[=] __device__(size_t ni, auto x, auto y, auto ndeg) {
                    // Skip leaked particles
                    if (x[ni] == 0 && y[ni] == 0 && ni < n)
                      return;
      ...
      exclusive_scan(
          ctx, lndeg, lndeg_sum,
          [] __device__(const int &a, const int &b) { return a + b; }, 0);

      setVal(ctx, lndeg, 0); // initialise again

      ctx.parallel_for(lx.shape(), lx.read(), ly.read(), ljaret.write(),
                       lndeg.rw(), lndeg_sum.read())
              .set_symbol("neighbors")
              ->*[=] __device__(size_t ni, auto x, auto y, auto jaret,
                                auto ndeg, auto ndeg_sum) {
                    // Skip leaked particles
                    if (x[ni] == 0 && y[ni] == 0 && ni < n)
                      return;
    ...
    // Particle loop
    ctx.parallel_for(box(n), lx.rw(), ly.rw(), lu.read(), lv.read(), lrho.rw(),
                     ldrho.rw(), ljaret.read(), lndeg.read(), lndeg_sum.read(),
                     lp.read(), lfx.rw(), lfy.rw())
            .set_symbol("particle loop")
            ->*
        [=] __device__(size_t ni, auto x, auto y, auto u, auto v, auto rho,
                       auto drho, auto jaret, auto ndeg, auto ndeg_sum, auto p,
                       auto fx, auto fy) {
          // Skip leaked particles
          if (x[ni] == 0 && y[ni] == 0)
            return;
          ...
          for (int psi = 0; psi < stencil_size; psi++) {
            ...
            for (int k = 0; k < ndeg[i * cb + j]; k++) {
              ...
              if (circleCheck(difx, dify, Dx)) {
                ...
                double muj = mu;
                double visc_termx =
                    viscTensor(du, dr0, mui, muj, rho[ni], rho[qi], dwr, h);
                double visc_termy =
                    viscTensor(dv, dr0, mui, muj, rho[ni], rho[qi], dwr, h);

                // Calculate sumation terms
                double sum_termx =
                    mass *
                    ((p[ni] / rho[ni] / rho[ni] + p[qi] / rho[qi] / rho[qi]) *
                         weightx +
                     visc_termx);
                double sum_termy =
                    mass *
    ...
    ctx.parallel_for(lrho.shape(), lrho.rw(), lrho_old.rw(), ldrho.rw(),
                     lp.write(), lfx.write(), lx.write(), lfy.write(),
                     ly.write(), lu.write(), lv.write(), lu_old.rw(),
                     lv_old.rw(), lx_old.rw(), ly_old.rw(),
                     lmaxrho.reduce(reducer::maxval<double>{}))
            .set_symbol("update")
            ->*
        [=] __device__(size_t ni, auto rho, auto rho_old, auto drho, auto p,
                       auto fx, auto x, auto fy, auto y, auto u, auto v,
                       auto u_old, auto v_old, auto x_old, auto y_old,
                       auto &maxrho) {
    ...
    if (t_sim / (1 / fps) > print_count) {
      print2file(ctx, lx, pathx);
      print2file(ctx, ly, pathy);
